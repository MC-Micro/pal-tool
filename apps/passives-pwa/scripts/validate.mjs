...
const requiredFiles = [
  'index.html', 'app.css', 'app.js', 'data-passives.js', 'data-overrides.js',
...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(appRoot, file))) fail(`Pflichtdatei fehlt: ${file}`);
}

for (const file of ['app.js', 'sw.js', 'data-passives.js', 'data-overrides.js']) {
  if (!fs.existsSync(path.join(appRoot, file))) continue;
  try {
...
try {
  vm.runInContext(fs.readFileSync(path.join(appRoot, 'data-passives.js'), 'utf8'), context, { filename: 'data-passives.js' });
  vm.runInContext(fs.readFileSync(path.join(appRoot, 'data-overrides.js'), 'utf8'), context, { filename: 'data-overrides.js' });
} catch (error) {
...
try {
  ...
  for (const icon of manifest.icons || []) {
    if (!fs.existsSync(path.join(appRoot, icon.src))) fail(`Manifest verweist auf fehlendes Icon: ${icon.src}`);
  }
} catch (error) {
...
for (const file of requiredFiles) {
  if (file !== 'sw.js' && !serviceWorker.includes(`./${file}`)) fail(`sw.js cached ${file} nicht.`);
}
if (!serviceWorker.includes('palworld-passives-pwa-v1.0.0-meta1')) fail('Der eingefrorene Cache-Namespace wurde unerwartet geändert.');

if (errors.length) {
  console.error('\nLegacy-PWA-Validierung fehlgeschlagen:\n');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log('Legacy-PWA-Validierung erfolgreich: eingefrorener 102er Overlay, App-Dateien und Cache-Verweise sind konsistent.');
